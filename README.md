...
<head>
  ...
  <!-- Firebase SDKs for Real-time Cloud Synchronization -->
...
<body class="fun-bg text-slate-800 min-h-screen flex flex-col font-sans antialiased selection:bg-pkc-yellow selection:text-pkc-navy relative overflow-x-hidden">
  ...
  <header class="bg-gradient-to-r from-pkc-navy via-blue-950 to-pkc-navy text-white shadow-xl border-b-4 border-pkc-yellow relative z-10">
    <div class="max-w-7xl mx-auto px-4 py-4 sm:px-6 lg:px-8 flex flex-col md:flex-row items-center justify-between gap-4">
      ...
      <div class="flex items-center gap-3 text-center sm:text-left">
        ...
        <div>
          <div class="flex items-center gap-2 justify-center sm:justify-start">
            ...
            <button id="syncIndicator" class="bg-amber-500 hover:bg-amber-600 text-white text-[10px] font-black uppercase px-2.5 py-0.5 rounded-full shadow border border-amber-300 flex items-center gap-1 cursor-pointer transition">
              <i class="fa-solid fa-hard-drive text-[9px]"></i> Local Storage (Setup Cloud)
            </button>
          </div>
      ...
      <div class="flex items-center gap-2 flex-wrap justify-center">
        ...
        <!-- Firebase Config Setup Button -->
        <button id="openCloudConfigBtn" class="bg-blue-900/80 hover:bg-blue-800 text-amber-300 font-bold text-xs sm:text-sm px-3 py-2 rounded-xl transition duration-200 border border-blue-600/60 shadow flex items-center gap-1.5" title="Configure Cloud Sync">
          <i class="fa-solid fa-cloud text-amber-400"></i> Cloud Setup
        </button>
  ...
  <!-- CLOUD CONFIG SETTINGS MODAL -->
  <div id="firebaseSettingsModal" class="fixed inset-0 bg-slate-950/70 backdrop-blur-md z-50 flex items-center justify-center p-4 hidden opacity-0 transition-all duration-300">
    <div class="bg-white rounded-3xl shadow-2xl max-w-lg w-full p-6 border-4 border-pkc-yellow relative overflow-hidden">
      <div class="flex items-center justify-between mb-4 pb-2 border-b border-slate-200">
        <h3 class="text-xl font-black text-pkc-navy flex items-center gap-2">
          <i class="fa-solid fa-cloud-arrow-up text-amber-500"></i> Real-time Cloud Sync Setup
        </h3>
        <button id="closeCloudModalBtn" class="text-slate-400 hover:text-slate-600 font-bold text-lg p-1">❌</button>
      </div>

      <p class="text-xs text-slate-600 mb-4">
        To enable real-time synchronization across multiple devices on GitHub Pages, paste your <strong>Firebase Project Credentials JSON</strong> or object below.
      </p>
      ...
      <div class="bg-blue-50 border border-blue-200 rounded-xl p-3 mb-4 text-xs text-blue-900">
        <p class="font-bold flex items-center gap-1 mb-1">
          <i class="fa-solid fa-lightbulb text-amber-500"></i> Free Firebase Setup (1 Minute):
        </p>
        <ol class="list-decimal list-inside space-y-0.5 text-[11px]">
          <li>Create a free project on <a href="https://console.firebase.google.com" target="_blank" class="underline font-bold text-pkc-blue">Firebase Console</a></li>
      ...
      <div class="space-y-3">
        <div>
          <label for="fbConfigJson" class="block text-xs font-bold text-slate-700 uppercase mb-1">Firebase Configuration JSON / Object</label>
          <textarea id="fbConfigJson" rows="5" placeholder='{\n  "apiKey": "AIzaSy...",\n  "authDomain": "your-app.firebaseapp.com",\n  "projectId": "your-app-id"\n}' class="w-full px-3 py-2 text-xs font-mono border border-slate-300 rounded-xl focus:ring-2 focus:ring-pkc-blue focus:outline-none bg-slate-50"></textarea>
        </div>
      </div>

      <div class="flex gap-2 mt-5">
        <button id="saveCloudConfigBtn" class="flex-1 bg-emerald-600 hover:bg-emerald-700 text-white font-black py-2.5 rounded-xl text-xs transition shadow flex items-center justify-center gap-1.5">
          <i class="fa-solid fa-check"></i> Save & Enable Live Sync
        </button>
        <button id="clearCloudConfigBtn" class="bg-red-50 hover:bg-red-100 text-red-600 font-bold px-3 py-2.5 rounded-xl text-xs border border-red-200 transition">
          Clear Config
        </button>
      </div>
    </div>
  ...
  <script>
    // RETRIEVE OR LOAD FIREBASE CONFIGURATION
    function getSavedFirebaseConfig() {
      if (typeof __firebase_config !== 'undefined' && __firebase_config) {
        try { return JSON.parse(__firebase_config); } catch(e){}
      }
      const stored = localStorage.getItem('pkc_firebase_config');
      if (stored) {
    ...
    let rawFirebaseConfig = getSavedFirebaseConfig();
    let db = null;
    let auth = null;
    let isCloudEnabled = false;

    function initFirebaseSDK() {
      rawFirebaseConfig = getSavedFirebaseConfig();
      if (typeof firebase !== 'undefined' && rawFirebaseConfig && rawFirebaseConfig.apiKey) {
        try {
    ...
    async function init() {
      ...
      if (isCloudEnabled && db && auth) {
        try {
          ...
          console.warn("Auth failed, using Local Storage mode", e);
        }
      }
    ...
    function updateSyncStatusUI() {
      ...
      if (isCloudEnabled) {
        syncIndicator.className = "bg-emerald-500 hover:bg-emerald-600 text-white text-[10px] font-black uppercase px-2.5 py-0.5 rounded-full shadow border border-emerald-300 flex items-center gap-1 cursor-pointer transition animate-pulse";
        syncIndicator.innerHTML = `<i class="fa-solid fa-wifi text-[9px]"></i> Live Sync ON`;
      } else {
        syncIndicator.className = "bg-amber-500 hover:bg-amber-600 text-white text-[10px] font-black uppercase px-2.5 py-0.5 rounded-full shadow border border-amber-300 flex items-center gap-1 cursor-pointer transition";
        syncIndicator.innerHTML = `<i class="fa-solid fa-hard-drive text-[9px]"></i> Local Storage (Setup Cloud)`;
      }
    }
    ...
    function setupFirestoreListener() {
      ...
      const docRef = db.collection('pkc_lunch_rooms').doc(currentRoomCode);
    ...
    function pushToCloud() {
      saveToLocalStorage();
      if (!isCloudEnabled || !db || isRemoteUpdate) return;
    ...
    function setupEventListeners() {
      ...
      document.getElementById('syncIndicator').addEventListener('click', openCloudModal);
      document.getElementById('openCloudConfigBtn').addEventListener('click', openCloudModal);
      ...
      document.getElementById('saveCloudConfigBtn').addEventListener('click', () => {
        const val = document.getElementById('fbConfigJson').value.trim();
        if (!val) {
          showToast('Please enter a valid Firebase JSON configuration!', '⚠️');
          return;
        }
        try {
          const parsed = JSON.parse(val);
          if (!parsed.apiKey || !parsed.projectId) {
            showToast('JSON must contain apiKey and projectId fields.', '⚠️');
            return;
          }
          localStorage.setItem('pkc_firebase_config', JSON.stringify(parsed));
          closeCloudModal();
          showToast('Cloud credentials saved! Re-initializing...', '☁️');
      ...
      document.getElementById('clearCloudConfigBtn').addEventListener('click', () => {
        localStorage.removeItem('pkc_firebase_config');
        closeCloudModal();
        showToast('Cleared cloud configuration.', '🗑️');
        setTimeout(() => window.location.reload(), 1000);
      });
